...
const ProfessorGradePage = () => {
  ...
  const handleRegisterOrUpdate = async (studentId, enrollmentId, gradeId) => {
    ...
    try {
      ...
      if (gradeId) {
        await updateGrade(dto);
        showModal("성적 수정 완료");
      } else {
        await registerGrade(dto);
        showModal("성적 등록 완료");
      }
      fetchGrades(selectedClassId);
    } catch (err) {
      showModal("성적 등록/수정에 실패했습니다.", "error");
    }
  };
